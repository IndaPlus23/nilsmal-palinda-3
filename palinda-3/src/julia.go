...
func main() {
	channel := make(chan string, 1) // Make room for one unmatched send.
	wg := new(sync.WaitGroup)
	wg.Add(len(Funcs))
	for n, fn := range Funcs {
		go Init(fn, n, channel, wg)
	}
	wg.Wait()
}

func Init(fn ComplexFunc, n int, channel chan string, wg *sync.WaitGroup) {
